...
// // 🆕 Register user (sirf admin / Postman se use karo)
...
// // 🔑 Login (frontend yahi use karega)
...
// /* ------------------------ CLEARBOOKS OAUTH (TUMHARA PURANA CODE) ------------------------ */

// // ✅ Step 1: Redirect to ClearBooks OAuth
...
//   }&redirect_uri=${encodeURIComponent(process.env.REDIRECT_URI)}&scope=${encodeURIComponent(scopes)}`;

//   res.redirect(authUrl);
// });

// // ✅ Step 2: Handle callback, get tokens, and fetch businesses
// router.get("/callback", async (req, res) => {
//   const code = req.query.code;
//   if (!code) return res.status(400).send("Missing authorization code");

//   try {
//     console.log("🔁 Received code from ClearBooks:", code);
...
//     const tokenRes = await axios.post(process.env.TOKEN_URL, params.toString(), {
...
//     // 2️⃣ Business list le aao
//     console.log("🔍 Fetching businesses...");
//     const bizRes = await axios.get("https://api.clearbooks.co.uk/v1/businesses", {
...
//       return res.json({
//         success: false,
//         message: "No businesses found in your ClearBooks account.",
//       });
//     }

//     // 3️⃣ First business select karo
//     const selected = businesses[0];
//     console.log(`✅ Selected business: ${selected.name || "Unnamed"} (${selected.id})`);

//     // 4️⃣ YAHAN business_id token ke andar save karo
//     tokenData.business_id = selected.id;

//     // 5️⃣ Ab tokens.json likho (access + business_id dono)
//     fs.writeFileSync("tokens.json", JSON.stringify(tokenData, null, 2));
//     console.log("✅ Access Token + Business ID Saved:", tokenData);

//     // 6️⃣ Frontend pe redirect
//     return res.redirect(
//       `http://localhost:5173/?auth=success&company=${encodeURIComponent(selected.name)}`
//     );
//   } catch (err) {
...
// // 🔌 Disconnect ClearBooks (delete tokens + clear business id)
// router.post("/disconnect", async (req, res) => {
//   try {
//     // 1) tokens.json hata do
//     if (fs.existsSync("tokens.json")) {
//       fs.unlinkSync("tokens.json");
//       console.log("🗑️ tokens.json deleted");
//     }

//     // 2) .env se CLEARBOOKS_BUSINESS_ID hatao / blank karo
//     if (fs.existsSync(".env")) {
//       const envFile = fs.readFileSync(".env", "utf8");
//       let lines = envFile.split(/\r?\n/);

//       // ya to line remove kar do...
//       // lines = lines.filter((l) => !l.startsWith("CLEARBOOKS_BUSINESS_ID="));

//       // ...ya blank set kar do:
//       let found = false;
//       lines = lines.map((l) => {
//         if (l.startsWith("CLEARBOOKS_BUSINESS_ID=")) {
//           found = true;
//           return "CLEARBOOKS_BUSINESS_ID=";
//         }
//         return l;
//       });

//       if (!found) {
//         lines.push("CLEARBOOKS_BUSINESS_ID=");
//       }

//       fs.writeFileSync(".env", lines.join("\n"));
//       console.log("🧹 CLEARBOOKS_BUSINESS_ID cleared in .env");
//     }

//     return res.json({
//       success: true,
//       message: "Disconnected from ClearBooks. Please reconnect to use the dashboard.",
//     });
//   } catch (err) {
